...
void UpdatePawn(int x, int y, Figure board[8][8], Game* game) {
	...
	if (x > 250 && x < 750) { //wyszukanie pozycji napisu przedzia³y dla osi x
		if (y > 275 && y < 350) // przedzia³y osi y dla wyboru Hetmana
			updateType = Queen;
		else if (y > 350 && y < 415) // przedzia³y osi y dla wyboru goñca 
			updateType = Bishop;
		else if (y > 415 && y < 480) // przedzia³y osi y dla wyboru konia
			updateType = Knight;
		else if (y > 480 && y < 560) // przedzia³y osi y dla wyboru wie¿y
			updateType = Rook;
		else
			return;
	}
